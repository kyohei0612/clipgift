...
## 2026-08-04 プレビューの当たり判定・再生設定の記憶（v2.0.19）

- **[DB-79] 🔴 プレビューでコメントが重なる（当たり判定が無かった）** — [templates/index2.html](templates/index2.html)
  - 報告: 「こういう感じでコメント重なるけど計算違うんじゃない？早さとかで後から行流したりするやつもあるし」
  - 原因: プレビューは **y を完全ランダムに置くだけ**で、レーン管理も追突判定も無かった。
    ```js
    const y = yMin + Math.random() * Math.max(0, yMax - yMin);
    ```
    書き出し側（mp4inchatnagasi.py）にはレーン制と `next_free_time` を入れたが、
    プレビューには入れていなかった。さらに DB-76 で流量を実データ基準にしたため
    重なりが目立つようになった。**仕上がりを判断するためのプレビューが仕上がりを表していない**状態。
  - 対応: 書き出しと同じレーン配置と追突判定を移植。
    `必要な間隔 = 表示時間 × TW / (画面幅 + TW)`（TW = max(前の幅, 今回の幅)）。
    空きレーンが無ければ流さない（書き出しと同じ挙動）。フォント/サイズ変更時は
    レーン占有をリセット（古い幅で判定しないため）。
  - 定数はプレビューと書き出しで一致することを機械的に確認
    （画面端 50px / 上下 10px / 余裕 0.15 秒 / 表示 7 秒）。
  - 検証: 実 CSV（秒 3.19 件）で 60 秒ぶんシミュレートし全ペア判定。
    レーン 14 本 / 配置 192 件 / 隣接ペア 178 組 / **重なり 0 件 / 最小マージン +14.2px**。

- **[DB-80] 再生速度・音量を次回起動まで覚える** — [templates/index2.html](templates/index2.html)
  - 要望: 「倍速とか速度いじったらそれも次回起動時残しといて」
  - 実装: `PlaybackPrefs` を新設。`ratechange` / `volumechange` を拾って localStorage に保存し、
    `loadedmetadata` のたびに復元する（動画を差し替えるとブラウザが既定値に戻すため）。
    不正値・範囲外（rate 0.25〜4 / volume 0〜1 の外）は無視。
  - ⚠️ **muted は保存しない**。クリップ再生時に自動再生ブロックを避けるため
    一時的に `muted=true` → 再生開始後 `false` に戻す処理があり、
    保存すると取り合いになって「勝手にミュートが戻る」事故になる。
  - 検証: 1.75 倍 / 音量 0.4 に変更 → ページ再読み込み後も復元されることを実機で確認。
  - ✅ **書き出しには影響しないことを実測で確認**（「倍速で DL されない？」への回答）。
    165〜195 秒（30 秒）を指定して書き出し → 出力は **30.00 秒 / 60fps / 音声 30.00 秒**
    （1.75 倍なら 17.1 秒になるはず）。構造的にも `playbackRate` の参照は index2.html 内の
    4 箇所（localStorage と `<video>` への適用）だけで、formData にも載らず、
    app.py / mp4inchatnagasi.py に速度関連の記述は無く、ffmpeg も `atempo` / `setpts` を使わない。
    書き出しは元動画を ffmpeg が直接読むので、プレイヤー設定とは無関係。

> 📌 検証メモ: **ポート 5001 はインストール版（`%LOCALAPPDATA%\ClipGift`）が握っていることがある。**
> ClipGift.exe が起動していると、リポジトリ版の `python app.py` は多重起動防止で終了し、
> 気づかずインストール版を検証してしまう。開発時は `CLIPGEN_PORT=5055` など別ポートを使うこと。
